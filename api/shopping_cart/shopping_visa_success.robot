...
*** Keywords ***
Search
    ${resp}=    Get Request    alias=shopping    headers=&{headers}    uri=/api/v1/product
    Request Should Be Successful    ${resp}
    Should Be Equal As Integers    ${resp.json()['products'][1]['id']}    2
...
Product Detail
    ${resp}=    Get Request    alias=shopping    headers=&{headers}    uri=/api/v1/product/2
    Request Should Be Successful    ${resp}
